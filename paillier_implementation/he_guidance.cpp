						...
						const uint32_t size)
{
	...
	// Init all stuff
	mpz_init_set(N, N_in);
	mpz_init_set(N2, N2_in);
	mpz_init_set(lambda, lambda_in);
	mpz_init_set(g, g_in);
	mpz_init_set(p2, p2_in);
	mpz_init_set(q2, q2_in);
	mpz_init_set(mu, mu_in);
...
void Encrypted_ilos_guidance::preprocessing(float *waypoints, size_t n)
{
	...
	paillier_encrypt(c_int_0, randstate, c_int_0, g, N, N2);
						...
						const float y_n)
{
	...
	paillier_encrypt(c_x, randstate, x_n_bar, g, N, N2);
	paillier_encrypt(c_y, randstate, y_n_bar, g, N, N2);
}
			       	...
			       	const mpz_t c_yn)
{
	...
	// homomorphically
	mpz_invert(inv, current_origin_x, N2);
	mpz_mul(c_yep, c_xn, inv);
	mpz_mod(c_yep, c_yep, N2);

	mpz_powm(c_yep, c_yep, current_sin_pi, N2);
	mpz_invert(c_yep, c_yep, N2);

	mpz_invert(inv, current_origin_y, N2);
	mpz_mul(tmp_x, c_yn, inv);
	mpz_mod(tmp_x, tmp_x, N2);
	mpz_powm(tmp_x, tmp_x, current_cos_pi, N2);

	mpz_mul(c_yep, c_yep, tmp_x);
	mpz_mod(c_yep, c_yep, N2);
	
	// Update integral
	mpz_powm(tmp_x, c_yep, delta_t, N2);
	mpz_mul(c_integral, c_integral, tmp_x);
	mpz_mod(c_integral, c_integral, N2);

	// Compute c_psi_d
	mpz_powm(tmp_x, c_yep, kp_bar, N2);
	mpz_invert(tmp_x, tmp_x, N2);
	mpz_powm(inv, c_integral, ki_bar, N2);
	mpz_invert(inv, inv, N2);
	mpz_mul(c_psi_d, current_c_pi, tmp_x);
	mpz_mod(c_psi_d, c_psi_d, N2);
	...
	mpz_powm(c_xep, x_tilde, current_cos_pi, N2);
	...
	mpz_mod(c_xe, c_xe, N2);
	mpz_mod(c_ye, c_ye, N2);
}
						...
						const mpz_t c_y_e)
{
	...
	paillier_decrypt(psi_d_bar, c_psi_d, lambda, mu, N, p2, q2);
	paillier_decrypt(x_e_bar, c_x_e, lambda, mu, N, p2, q2);
	paillier_decrypt(y_e_bar, c_y_e, lambda, mu, N, p2, q2);
